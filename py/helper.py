...
class DataProcessing(FastaMeta):
    ...
    def get_amino_df(self, k, meta_format):
        '''
        k = kmer length
        ...
        '''
        meta_df = self.get_meta_df(meta_format)
    ...
    def get_fasta__df(self):
        seq_seq = defaultdict(list)
        seq_list = list(SeqIO.parse(self.fasta, 'fasta'))
        print(seq_list[:1])
        for s in seq_list:
            seq_seq['seq'].append(str(s.seq).strip())
            seq_seq['id'].append(str(s.id).strip())
            seq_seq['description'].append(str(s.description).strip())
        return pd.DataFrame(seq_seq)
    ...
    def get_meta_df(self, format):
        formats = {'csv':',',
                    'tsv': '\t'}
        return pd.read_csv(self.meta, sep = formats.get(format))
